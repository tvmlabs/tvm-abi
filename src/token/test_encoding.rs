...
fn test_parameters_set(
    ...
    for version in versions {
        ...
        let params: Vec<Param> =
            if let Some(params) = params { params.to_vec() } else { params_from_tokens(inputs) };
...
fn tokens_from_values(values: Vec<TokenValue>) -> Vec<Token> {
    ...
    values
        .into_iter()
        .zip(param_names)
        .map(|(value, name)| Token { name: name.to_owned(), value })
        .collect()
}
...
fn test_with_address() {
    ...
    builder
        .checked_append_reference(
            BuilderData::with_bitstring(vec![1, 2, 3, 0x80]).unwrap().into_cell().unwrap(),
        )
        .unwrap();
    ...
    let mut builders: Vec<BuilderData> =
        addresses.iter().map(|address| address.write_to_new_cell().unwrap()).collect();
    builders.reverse();
...
fn test_empty_params() {
    ...
    test_parameters_set(&[], None, builder, &[ABI_VERSION_1_0, ABI_VERSION_2_0, ABI_VERSION_2_2]);
}
...
fn test_five_refs_v1() {
    ...
    builder
        .checked_append_reference(
            BuilderData::with_raw(vec![0x55; 127], 127 * 8).unwrap().into_cell().unwrap(),
        )
        .unwrap();
    ...
    test_parameters_set(&tokens_from_values(values), None, builder, &[ABI_VERSION_1_0]);
}
...
fn test_five_refs_v2() {
    ...
    builder
        .checked_append_reference(
            BuilderData::with_raw(vec![0x55; 46], 46 * 8).unwrap().into_cell().unwrap(),
        )
        .unwrap();
...
fn test_header_params() {
    ...
    builder.append_bit_zero().unwrap();
    builder.append_bit_one().unwrap();
    builder.append_raw(&public_key, ED25519_PUBLIC_KEY_LENGTH * 8).unwrap();
...
fn test_big_map_value() {
    ...
    let value_array =
        TokenValue::Array(ParamType::Tuple(params_from_tokens(&tuple_tokens)), vec![tuple]);
...
fn test_ref_type() {
    ...
    builder.checked_append_reference(ref_builder.clone().into_cell().unwrap()).unwrap();
    ...
    test_parameters_set(&tokens_from_values(values), None, builder, &[MAX_SUPPORTED_VERSION]);
}
...
fn test_four_optional_strings() {
    ...
    test_parameters_set(&tokens_from_values(values.clone()), None, builder, &[ABI_VERSION_2_2]);
    ...
    test_parameters_set(&tokens_from_values(values), None, builder, &[ABI_VERSION_2_1]);
}
...
fn test_default_values() {
    ...
    second
        .checked_append_reference(
            BuilderData::with_raw([0u8; 2].as_slice(), 15).unwrap().into_cell().unwrap(),
        )
        .unwrap();
    ...
    second.checked_append_reference(third.into_cell().unwrap()).unwrap();
    root.checked_append_reference(second.into_cell().unwrap()).unwrap();
...
fn test_wrong_layout() {
    ...
    builder
        .checked_append_reference(
            BuilderData::with_raw(456u64.to_be_bytes().as_slice(), 64)
                .unwrap()
                .into_cell()
                .unwrap(),
        )
