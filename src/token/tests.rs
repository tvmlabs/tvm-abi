...
mod tokenize_tests {
    ...
    fn test_tokenize_ints() {
        ...
        let params = vec![
            ...
            Param { name: "c".to_owned(), kind: ParamType::Int(32) },
            Param { name: "e".to_owned(), kind: ParamType::Uint(13) },
            Param { name: "f".to_owned(), kind: ParamType::Int(128) },
            Param { name: "g".to_owned(), kind: ParamType::Token },
            Param { name: "h".to_owned(), kind: ParamType::VarInt(16) },
            Param { name: "i".to_owned(), kind: ParamType::VarUint(32) },
        ];

        let expected_tokens = vec![
            Token { name: "a".to_owned(), value: TokenValue::Uint(Uint::new(123, 8)) },
            Token { name: "b".to_owned(), value: TokenValue::Int(Int::new(-456, 16)) },
            Token { name: "c".to_owned(), value: TokenValue::Int(Int::new(-0xabcdef, 32)) },
            Token { name: "e".to_owned(), value: TokenValue::Uint(Uint::new(789, 13)) },
            Token {
                name: "f".to_owned(),
    ...
    fn test_int_checks() {
        ...
        let params = vec![Param { name: "a".to_owned(), kind: ParamType::Int(64) }];
        ...
        let params = vec![Param { name: "a".to_owned(), kind: ParamType::VarInt(16) }];
    ...
    fn test_tokenize_arrays() {
        ...
        let params = vec![
            Param { name: "a".to_owned(), kind: ParamType::Array(Box::new(ParamType::Int(16))) },
            Param {
                name: "b".to_owned(),
        ...
        let bool_array2 =
            vec![TokenValue::Bool(true), TokenValue::Bool(true), TokenValue::Bool(false)];
            ...
            Token {
                ...
                value: TokenValue::Array(
                    ...
                    vec![
                        ...
                        TokenValue::Tuple(vec![
                            Token { name: "a".to_owned(), value: TokenValue::Bool(false) },
                            Token {
                                name: "b".to_owned(),
    ...
    fn test_tokenize_cell() {
        ...
        let params = vec![Param::new("a", ParamType::Cell), Param::new("b", ParamType::Cell)];
    ...
    fn test_tokenize_hashmap() {
        ...
        expected_tokens
            .push(Token::new("a", TokenValue::Map(ParamType::Int(8), ParamType::Uint(32), map)));
        ...
        expected_tokens
            .push(Token::new("b", TokenValue::Map(ParamType::Uint(32), ParamType::Uint(32), map)));
        ...
        expected_tokens.push(Token::new(
            ...
            TokenValue::Map(
                ParamType::Int(8),
                ParamType::Tuple(vec![
                    Param { name: "q1".to_owned(), kind: ParamType::Uint(32) },
                    Param { name: "q2".to_owned(), kind: ParamType::Int(8) },
                ]),
                map,
            ),
        ));
        ...
        expected_tokens
            .push(Token::new("d", TokenValue::Map(ParamType::Address, ParamType::Uint(32), map)));
    ...
    fn test_tokenize_address() {
        ...
        let params =
            vec![Param::new("std", ParamType::Address), Param::new("var", ParamType::Address)];
    ...
    fn test_tokenize_time() {
        ...
        let params = vec![
            Param { name: "a".to_owned(), kind: ParamType::Time },
            Param { name: "b".to_owned(), kind: ParamType::Time },
            Param { name: "c".to_owned(), kind: ParamType::Time },
            Param { name: "d".to_owned(), kind: ParamType::Time },
        ];

        let expected_tokens = vec![
            Token { name: "a".to_owned(), value: TokenValue::Time(123) },
            Token { name: "b".to_owned(), value: TokenValue::Time(456) },
            Token { name: "c".to_owned(), value: TokenValue::Time(0x789) },
            Token { name: "d".to_owned(), value: TokenValue::Time(0xffffffffffffffff) },
        ];
    ...
    fn test_tokenize_expire() {
        ...
        let params = vec![
            Param { name: "a".to_owned(), kind: ParamType::Expire },
            Param { name: "b".to_owned(), kind: ParamType::Expire },
            Param { name: "c".to_owned(), kind: ParamType::Expire },
            Param { name: "d".to_owned(), kind: ParamType::Expire },
        ];

        let expected_tokens = vec![
            Token { name: "a".to_owned(), value: TokenValue::Expire(123) },
            Token { name: "b".to_owned(), value: TokenValue::Expire(456) },
            Token { name: "c".to_owned(), value: TokenValue::Expire(0x789) },
            Token { name: "d".to_owned(), value: TokenValue::Expire(0xffffffff) },
        ];
    ...
    fn test_tokenize_pubkey() {
        ...
        let params =
            vec![Param::new("a", ParamType::PublicKey), Param::new("b", ParamType::PublicKey)];
        ...
        let expected_tokens = vec![
            ...
            Token {
                ...
                value: TokenValue::Ref(Box::new(TokenValue::Tuple(vec![
                    Token { name: "c".to_owned(), value: TokenValue::Bool(true) },
                    Token {
                        name: "d".to_owned(),
...
mod types_check_tests {
    ...
    fn test_type_check() {
        ...
        let tokens = vec![
            Token { name: "a".to_owned(), value: TokenValue::Uint(big_uint.clone()) },
            Token { name: "b".to_owned(), value: TokenValue::Int(big_int.clone()) },
            Token { name: "c".to_owned(), value: TokenValue::VarUint(32, 789u32.into()) },
            Token { name: "d".to_owned(), value: TokenValue::VarInt(16, 1000u32.into()) },
            ...
            Token {
                ...
                value: TokenValue::Tuple(vec![
                    Token { name: "a".to_owned(), value: TokenValue::Bool(true) },
                    Token { name: "b".to_owned(), value: TokenValue::Uint(big_uint.clone()) },
                ]),
            },
            Token { name: "k".to_owned(), value: TokenValue::Cell(Cell::default()) },
            Token { name: "l".to_owned(), value: TokenValue::Address(MsgAddress::AddrNone) },
            Token {
                name: "m1".to_owned(),
            ...
            Token { name: "t".to_owned(), value: TokenValue::String("123".to_owned()) },
            Token { name: "u".to_owned(), value: TokenValue::Optional(ParamType::Int(256), None) },
            Token {
                name: "v".to_owned(),
                value: TokenValue::Optional(
                    ParamType::Bool,
        ...
        let params = vec![
            ...
            Param { name: "c".to_owned(), kind: ParamType::VarUint(32) },
            Param { name: "d".to_owned(), kind: ParamType::VarInt(16) },
            ...
            Param { name: "k".to_owned(), kind: ParamType::Cell },
            Param { name: "l".to_owned(), kind: ParamType::Address },
            Param {
                name: "m1".to_owned(),
            ...
            Param { name: "n".to_owned(), kind: ParamType::Bytes },
            Param { name: "o".to_owned(), kind: ParamType::FixedBytes(3) },
            Param { name: "p".to_owned(), kind: ParamType::Token },
            Param { name: "q".to_owned(), kind: ParamType::Time },
            Param { name: "r".to_owned(), kind: ParamType::Expire },
            Param { name: "s".to_owned(), kind: ParamType::PublicKey },
            Param { name: "t".to_owned(), kind: ParamType::String },
            Param {
                name: "u".to_owned(),
                kind: ParamType::Optional(Box::new(ParamType::Int(256))),
            },
            Param { name: "v".to_owned(), kind: ParamType::Optional(Box::new(ParamType::Bool)) },
            Param { name: "w".to_owned(), kind: ParamType::Ref(Box::new(ParamType::String)) },
        ];

        assert_type_check(&tokens, &params);

        let mut tokens_wrong_type = tokens.clone();
        tokens_wrong_type[0] = Token { name: "a".to_owned(), value: TokenValue::Bool(false) };
        assert_not_type_check(&tokens_wrong_type, &params);

        let mut tokens_wrong_int_size = tokens.clone();
        tokens_wrong_int_size[0] =
            Token { name: "a".to_owned(), value: TokenValue::Uint(Uint::new(456, 30)) };
        ...
        tokens_wrong_tuple_type[9] = Token {
            name: "f".to_owned(),
            value: TokenValue::Tuple(vec![
                Token { name: "a".to_owned(), value: TokenValue::Int(big_int.clone()) },
                Token { name: "b".to_owned(), value: TokenValue::Uint(big_uint.clone()) },
            ]),
        };
